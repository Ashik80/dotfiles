...
bluetooth_power() {
    ...
    bt_power_choice="$(rofi_command "Bluetooth Power" "${bt_power_options[@]}")"
...
bluetooth_connect_to_device() {
    ...
    bt_sel_device_name="$(rofi_command "Select Device" "${bt_device_names[@]}")"
...
power_options() {
    ...
    choice="$(rofi_command "Power Options" "${power_options[@]}")"

    case $choice in
        "Power Off")
            if ! poweroff 2>/dev/null; then
                password="$(rofi_input "Enter Password:")"
...
CHOICE="$(rofi_command "Utility Launcher" "${OPTIONS[@]}")"

case $CHOICE in
    "Wifi")
        if ! command -v impala &> /dev/null; then
            ~/dotfiles/projscripts/wnm.sh
        else
            st -e impala
        fi
        ;;
    ...
    "Bluetooth")
        ...
        choice="$(rofi_command "Bluetooth" "${options[@]}")"
